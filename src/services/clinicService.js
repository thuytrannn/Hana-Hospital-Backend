    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (!data.name || !data.descriptionHTML
                || !data.descriptionMarkdown || !data.address || !data.image) {
                resolve({
                    errCode: 1,
...
let saveInforClinicService = (data) => {
    return new Promise(async (resolve, reject) => {
        try {
            if (!data.descriptionHTML || !data.descriptionMarkdown || !data.clinicId || !data.address || !data.image) {
                resolve({
                    errCode: 1,
                    errMessage: 'Missing required parameters.',
                })
